...
function updateExtensionButtons() {
  ...
  const guidedSwipeButton = createActionButton('gg_swipe_button', '指导回复', 'fa-solid fa-forward', guidedSwipe); // Guided Swipe -> Instructed Reply
  actionButtonsContainer.appendChild(guidedSwipeButton);

  // Guided Response Button (Restore correct icon)
  const guidedResponseButton = createActionButton('gg_response_button', '指导响应', 'fa-solid fa-dog', guidedResponse); // Guided Response -> Instructed Response
  ...
  if (settings.showGuidedContinue) {
    const guidedContinueButton = createActionButton(
      ...
      guidedContinue,
    ); // Guided Continue -> Instructed Continue
